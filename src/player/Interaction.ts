...
import { B, BREAK_TIME, HOTBAR, NAMES, SOUND_CAT, CROSS, tileFor } from '../world/Blocks'
...
export class Interaction {
  ...
  constructor(world: World, player: Player, camera: THREE.PerspectiveCamera, scene: THREE.Scene, atlas: Atlas, audio: AudioMan, particles: Particles) {
    this.world = world
    this.player = player
    this.camera = camera
    this.atlas = atlas
  ...
  get selectedBlock(): number { return HOTBAR[this.selected] }
  get selectedName(): string { return NAMES[this.selectedBlock] }
  ...
  private buildHand(): void {
    ...
    // remap each face's uvs onto its atlas tile
    const uv = geo.getAttribute('uv') as THREE.BufferAttribute
    // BoxGeometry face order: +x, -x, +y, -y, +z, -z — 4 uv verts each
    for (let f = 0; f < 6; f++) {
    ...
    uv.needsUpdate = true
    // vertex colors: grass tops need their biome tint baked in
    ...
    this.hand.position.set(0.3, -0.26, -0.55)
    this.hand.rotation.set(0.12, Math.PI / 5, 0)
  ...
  update(dt: number): void {
    ...
    if (this.breaking && hit) {
      ...
      if (isFinite(time)) {
        ...
        const frac = this.breakProgress / time
        if (frac >= 1) {
          const id = hit.id
    ...
    if (this.placing && hit && this.placeCooldown <= 0) {
      ...
      if (replaceable && !this.player.intersectsBlock(px, py, pz)) {
        this.world.setBlock(px, py, pz, this.selectedBlock)
        this.audio.placeBlock(SOUND_CAT[this.selectedBlock])
        this.swing()
