...
const bundle = buildSync({
  stdin: {
    contents: [
      "export { waterCellHeight, fallDamageForHeight } from './src/player/Player.ts'",
...
const { waterCellHeight, fallDamageForHeight, B } = bundledModule.exports
...
test('short fractional terrain drops do not round up to fall damage', () => {
  assert.equal(fallDamageForHeight(3), 0)
  assert.equal(fallDamageForHeight(3.99), 0)
  assert.equal(fallDamageForHeight(4), 1)
  assert.equal(fallDamageForHeight(5.9), 2)
})
